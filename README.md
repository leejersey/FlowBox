...
> 数据可靠性、安全存储、原生集成与桌面体验升级

- ✨ **番茄 Mini 模式**：主窗口可切换为置顶紧凑计时器；退出、切换路由或卸载时恢复原尺寸、位置、最大化及置顶状态
- ✨ **原生设置入口**：侧栏设置项精简为可访问的齿轮按钮；macOS 应用菜单新增“设置…”与 `Cmd+,`，可唤醒主窗口并打开偏好设置
- ✨ **跨模块关联完善**：待办、灵感、语音、剪贴板支持双向创建、查看、跳转与删除关联
- ✨ **OCR 分流**：OpenAI 直接使用 Vision；DeepSeek / Ollama 通过 macOS Vision OCR 提取文本后再整理
- ✨ **Dev Trending**：热门仓库浏览、语言筛选、AI 中文摘要及一键收藏到灵感
- 🔐 **安全与系统集成**：AI / ASR 凭据迁移至 macOS Keychain，收紧 CSP，并使用官方 Tauri 插件管理开机自启动
- 🧭 **数据与生命周期修复**：统一本地日期边界、每日回顾、番茄 running 状态、统计结算、后台同步及应用使用时长增量
- ⚡ **性能与界面体验**：功能路由按需加载，补充详情面板、可调整布局、右键菜单与交互音效
- 🧪 **发布质量门禁**：统一版本为 0.5.0，新增 macOS CI、前后端测试、类型检查、Rust 检查与依赖安全审计
- 🗄️ **数据库迁移**：新增番茄 running、应用使用唯一约束及关联规范化与孤儿清理迁移
